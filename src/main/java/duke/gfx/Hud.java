...
public class Hud {
    ...
    private void drawBorders(Renderer renderer) {
        ...
        renderer.draw(border.get(4), 0, 0);
        renderer.draw(border.get(7), 0, 176);
        renderer.draw(border.get(9), 304, 0);
        renderer.draw(border.get(11), 304, 176);
        renderer.draw(border.get(5), 224, 0);
        renderer.draw(border.get(6), 224, 176);

        for (int i = 1; i < 14; i++) {
            renderer.draw(border.get(2), i * TILE_SIZE, 0);
            renderer.draw(border.get(3), i * TILE_SIZE, 176);

            if (i < 11) {
                renderer.draw(border.get(0), 0, i * TILE_SIZE);
                renderer.draw(border.get(1), 224, i * TILE_SIZE);
                renderer.draw(border.get(10), 304, i * TILE_SIZE);
            }
        }

        drawScoreLabel(renderer);
        drawHealthLabel(renderer, false);
        drawFirepowerLabel(renderer);
        drawInventoryLabel(renderer);
        drawHelpLabel(renderer);
    }

    private void drawScoreLabel(Renderer renderer) {
    ...
    private void drawHealthLabel(Renderer renderer, boolean flash) {
    ...
    private void drawInventoryLabel(Renderer renderer) {
    ...
    private void drawHelpLabel(Renderer renderer) {
        int screenX = 88;
