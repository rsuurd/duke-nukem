package duke.level.processors;

import duke.gameplay.active.Reactor;
import duke.level.Level;
import duke.level.LevelBuilder;

import static duke.level.LevelBuilder.LEFT;

public class ReactorProcessor implements ActiveProcessor {
    static final int TILE_ID = 0x302b;

    @Override
    public boolean canProcess(int tileId) {
        return tileId == TILE_ID;
    }

    @Override
    public void process(int index, int tileId, LevelBuilder builder) {
        int x = Level.toX(index);
        int y = Level.toY(index) - 2 * Level.TILE_SIZE;

        builder.add(new Reactor(x, y)).replaceTile(index, LEFT);
    }
}
