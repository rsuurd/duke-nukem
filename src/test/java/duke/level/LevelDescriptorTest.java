...
class LevelDescriptorTest {
    ...
    void shouldIndicateHallway() {
        assertThat(new LevelDescriptor(0, 1, 0).isHallway()).isFalse();
        assertThat(new LevelDescriptor(1, 2, 0).isHallway()).isTrue();
    }

    @Test
    void shouldCreate() {
        LevelDescriptor descriptor = new LevelDescriptor(0, 1, 2, 3, "Hello, World!");

        assertThat(descriptor.index()).isEqualTo(0);
